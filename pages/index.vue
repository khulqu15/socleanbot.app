    ...
    <div class="container md:px-12 px-8 mx-auto max-w-full">
      <div class="flex gap-4 min-h-screen relative items-center justify-center">
        ...
        <div class="text-center relative">
          <img src="~/assets/img/robot.png" class="inline-block md:w-3/4 w-full absolute md:-left-72 left-0 md:-top-24 -top-32" alt="socleanbot">
          <form @submit.prevent="login($event)" class="rounded-xl md:w-3/4 w-full p-12 relative bg-black/40 shadow backdrop-blur-2xl md:left-32 md:top-0 top-32 text-left space-y-6">
            ...
            <button class="btn bg-gradient-to-bl from-cyan-500 to-blue-600 text-black w-full">Login</button>
          </form>
        </div>
      </div>
    </div>
...
const login = (e) => {
  if(form.email === 'dev@siderobot.id' && form.password === 'password') {
    return navigateTo({
      path: '/connect'
    })
  }
